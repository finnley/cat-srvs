...
class ConsulRegister(base.Register):
    ...
    def register(self, service_name, service_id, address, port, tags, check) -> bool:
        if check is None:
            check = {
                "GRPC": f"{address}:{port}",
                "GRPCUseTLS": False,
        ...
        return self.c.agent.service.register(name=service_name, service_id=service_id, address=address, port=port, tags=tags, check=check)
